...
static void Confirm( string product, uint count, string name, string address )
{
    ...
    Console.Write("Введите 'Да', если всё верно: ");
...
{
    DateTime thisDayPlus3 = DateTime.Today.AddDays( 3 );
    Console.WriteLine( $"{name}! Ваш заказ {product} в количестве {count} оформлен! Ожидайте доставку по адресу {address} к {thisDayPlus3.ToString( "D" )}" );
}
